...
public class TileEntityDispenser extends TileEntityLockableLoot
{
    ...
    {
        if (CarpetSettings.autoCraftingDropper && this.world != null)
        {
            IBlockState state = this.world.getBlockState(this.pos);
            if (state.getBlock() == Blocks.DROPPER)
            {
                if (this.world.getBlockState(this.pos.offset(state.getValue(BlockDispenser.FACING))).getBlock() == Blocks.CRAFTING_TABLE)
                {
