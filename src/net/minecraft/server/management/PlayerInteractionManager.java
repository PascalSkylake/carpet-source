...
public class PlayerInteractionManager
{
    ...
    {
        boolean worldEditResult = WorldEditBridge.onLeftClickBlock(world, pos, player);
        if (!worldEditResult)
        {
        ...
        else
        {
            ...
            else
            {
                ...
                if(CarpetSettings.duplicationFixUpdateSuppression) {
                    flag1 = world.isRemovableBlock(pos);
                } else {
                    playerMinedBlock = player;
    ...
    public EnumActionResult processRightClickBlock(EntityPlayer player, World worldIn, ItemStack stack, EnumHand hand, BlockPos pos, EnumFacing facing, float hitX, float hitY, float hitZ)
    {
        ...
        {
            boolean worldEditResult = WorldEditBridge.onRightClickBlock(world, pos, this.player);
            if (!worldEditResult)
            {
            ...
            if (!player.isSneaking() || player.getHeldItemMainhand().isEmpty() && player.getHeldItemOffhand().isEmpty())
            {
                ...
                WorldHelper.getMultimeter().logInteractBlock(worldIn, pos); // RSMM
