...
public class EntityLlama extends AbstractChestHorse implements IRangedAttackMob
{
    ...
    protected boolean handleEating(EntityPlayer player, ItemStack stack)
    {
        ...
        else if (item == Item.getItemFromBlock(Blocks.HAY_BLOCK))
        {
            ...
            if (this.isTame() && this.getGrowingAge() == 0 && !(CarpetSettings.llamaOverfeedingFix && this.isInLove() ) )
            {
                flag = true;
    ...
    protected boolean isValidBreedingItem(ItemStack itemstack){
    	...
    	if(item == Items.WHEAT) return false;
    	else if(item == Item.getItemFromBlock(Blocks.HAY_BLOCK)) return false;
