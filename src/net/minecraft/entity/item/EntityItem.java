...
public class EntityItem extends Entity
{
    ...
    public boolean handleWaterMovement()
    {
        if(carpet.carpetclient.CarpetClientChunkLogger.logger.enabled)
            carpet.carpetclient.CarpetClientChunkLogger.setReason("Item checking if pushed by water: " + getName());
    ...
    public void onCollideWithPlayer(EntityPlayer entityIn)
    {
        if (!this.world.isRemote)
        {
            ...
            if(CarpetSettings.craftingWindowDuplication && entityIn instanceof EntityPlayerMP){
                ((EntityPlayerMP)entityIn).dupeItemScan(true);
            }
            ...
            if (this.pickupDelay == 0 && (this.owner == null || 6000 - this.age <= 200 || this.owner.equals(entityIn.getName())) && entityIn.inventory.addItemStackToInventory(itemstack))
            {
                ...
                // CM add meta
                entityIn.addStat(StatHelper.getObjectsPickedUpStats(item, itemstack.getMetadata()), i);
            }
            CarpetServer.playerInventoryStacking = false;
            if(CarpetSettings.craftingWindowDuplication && entityIn instanceof EntityPlayerMP){
                ((EntityPlayerMP)entityIn).dupeItemScan(false);
            }
        }
    }
