...
public class EntityAIAvoidEntity<T extends Entity> extends EntityAIBase
{
    ...
    public boolean shouldExecute()
    {
        ...
        boolean log = CarpetSettings.displayMobAI && this.entity instanceof EntityCreeper;
        if (list.isEmpty())
        {
            if (log)
            {
                this.entity.setCustomNameTag("Won't flee - no hazard");
            }
            return false;
        }
        else
        {
            ...
            if (vec3d == null)
            {
                if (log)
                {
                    this.entity.setCustomNameTag("Won't flee - no path");
                }
                return false;
            }
            else if (this.closestLivingEntity.getDistanceSq(vec3d.x, vec3d.y, vec3d.z) < this.closestLivingEntity.getDistanceSq(this.entity))
            {
                if (log)
                {
                    this.entity.setCustomNameTag(String.format("Won't flee - distance to close to %.2f, %.2f, %.2f", vec3d.x, vec3d.y, vec3d.z));
                }
                return false;
            }
            else
            {
                ...
                if (log)
                {
                    if (this.path == null)
                    {
                        this.entity.setCustomNameTag(String.format("Won't flee - path to %.2f, %.2f, %.2f doesn't exist", vec3d.x, vec3d.y, vec3d.z));
                        this.entity.world.setBlockState(new BlockPos(vec3d).down(), Blocks.STONE.getDefaultState(), 130);
                    }
                    else
                    {
                        this.entity.setCustomNameTag(String.format("Flees to %.2f, %.2f, %.2f", vec3d.x, vec3d.y, vec3d.z));
                        this.entity.world.setBlockState(new BlockPos(vec3d).down(), Blocks.EMERALD_BLOCK.getDefaultState(), 130);
                    }
    ...
    public String getTask() {
        return ": " + entity.getName() + " Task: Avoiding other entity";
    }
}