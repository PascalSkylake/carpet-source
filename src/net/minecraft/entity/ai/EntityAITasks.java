...
public class EntityAITasks
{
    ...
    public void onUpdateTasks()
    {
        ...
        if (this.tickCount++ % this.tickRate == 0)
        {
            for (EntityAITasks.EntityAITaskEntry entityaitasks$entityaitaskentry : this.taskEntries)
            {
                if(carpet.carpetclient.CarpetClientChunkLogger.logger.enabled)
                    carpet.carpetclient.CarpetClientChunkLogger.setReason("Entity" + entityaitasks$entityaitaskentry.action.getTask());
                if (entityaitasks$entityaitaskentry.using)
                {
        ...
        if (!this.executingTaskEntries.isEmpty())
        {
            ...
            {
                if(carpet.carpetclient.CarpetClientChunkLogger.logger.enabled)
                    carpet.carpetclient.CarpetClientChunkLogger.setReason("Entity" + entityaitasks$entityaitaskentry2.action.getTask());
