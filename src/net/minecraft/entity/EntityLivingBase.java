...
public abstract class EntityLivingBase extends Entity
{
    ...
    {
        if(CarpetSettings.duplicationFixPortalEntitys && isDead) return false; // Fixes duplication when entity's are set to be removed and can still drop items.
        return !this.isChild();
    }
    ...
    public void addPotionEffect(PotionEffect potioneffectIn)
    {
        if (this.isPotionApplicable(potioneffectIn))
        {
            ...
            {
                PotionEffect newEffect = potioneffect.combine(potioneffectIn);
    ...
    protected void onFinishedPotionEffect(PotionEffect effect)
    {
        ...
        // CM
        if (CarpetSettings.effectsFix && effect.previous != null) {
            addPotionEffect(effect.previous);
        }
        // CM END
    }
    ...
    public boolean attackEntityFrom(DamageSource source, float amount)
    {
        ...
        else
        {
            ...
            if (this.getHealth() <= 0.0F)
            {
                DamageReporter.modify_damage(this, source, amount, 0.0f, "Already dead and can't take more damage"); //CM
                return false;
            }
            else if (source.isFireDamage() && this.isPotionActive(MobEffects.FIRE_RESISTANCE))
            {
                DamageReporter.modify_damage(this, source, amount, 0.0f, "Resistance to fire"); //CM
                return false;
            }
            else
            {
    ...
    protected float applyPotionDamageCalculations(DamageSource source, float damage)
    {
        ...
        else
        {
            if (this.isPotionActive(MobEffects.RESISTANCE) && source != DamageSource.OUT_OF_WORLD)
            {
                ...
                DamageReporter.modify_damage(this, source, damage, f / 25.0F, "Resistance status effect"); //CM
                damage = f / 25.0F;
            }
    ...
    protected void damageEntity(DamageSource damageSrc, float damageAmount)
    {
        if (!this.isEntityInvulnerable(damageSrc))
        {
            ...
            DamageReporter.modify_damage(this, damageSrc, previous_amount, damageAmount, //CM
                    String.format("Armour %.1f, Toughness %.1f", (float)this.getTotalArmorValue(), (float)this.getEntityAttribute(SharedMonsterAttributes.ARMOR_TOUGHNESS).getAttributeValue())); //CM
                ...
                if (!this.isInLava() || this instanceof EntityPlayer && ((EntityPlayer)this).capabilities.isFlying)
                {
                    ...
                    else
                    {
                        ...
                        if(carpet.carpetclient.CarpetClientChunkLogger.logger.enabled)
                            carpet.carpetclient.CarpetClientChunkLogger.setReason("Entity walking around: " + getName());
                        this.move(MoverType.SELF, this.motionX, this.motionY, this.motionZ);
    ...
    protected void collideWithNearbyEntities()
    {
        ...
        if(CarpetSettings.optimizedCollisionCancellations && !canBePushed() && !(this instanceof net.minecraft.entity.boss.EntityDragon)) return;
