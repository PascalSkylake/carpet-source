...
public abstract class Entity implements ICommandSender
{
    ...
    protected void setSize(float width, float height)
    {
        if (width != this.width || height != this.height)
        {
            ...
            if(CarpetSettings.growingUpWallJumpFix){
                BabyGrowingUp.carpetSetSize(this, width, height);
                return;
            }
    ...
    public void move(MoverType type, double x, double y, double z)
    {
        ...
        else
        {
            ...
            if (y != 0.0D)
            {
                int k = 0;
                if (optimize) { // CARPET-MASA
                    list1 = this.world.getCollisionBoxes(this, this.getEntityBoundingBox().expand(0, y, 0));
                }
            ...
            if (x != 0.0D)
            {
                int j5 = 0;
                if (optimize) { // CARPET-MASA
                    list1 = this.world.getCollisionBoxes(this, this.getEntityBoundingBox().expand(x, 0, 0));
                }
            ...
            if (z != 0.0D)
            {
                int k5 = 0;
                if (optimize) { // CARPET-MASA
                    list1 = this.world.getCollisionBoxes(this, this.getEntityBoundingBox().expand(0, 0, z));
                }
    ...
    public boolean handleWaterMovement()
    {
        if(carpet.carpetclient.CarpetClientChunkLogger.logger.enabled)
            carpet.carpetclient.CarpetClientChunkLogger.setReason("Entity checking if pushed by water: " + getName());
    ...
    public NBTTagCompound writeToNBT(NBTTagCompound compound)
    {
        try
        {
            ...
            if(CarpetSettings.reloadSuffocationFix) {
                compound.setTag("AABBcarpet", this.newDoubleNBTList(getEntityBoundingBox().maxX, getEntityBoundingBox().maxY, getEntityBoundingBox().maxZ, getEntityBoundingBox().minX, getEntityBoundingBox().minY, getEntityBoundingBox().minZ));
            }
    ...
    public void readFromNBT(NBTTagCompound compound)
    {
        try
        {
            ...
            // Save hitbox of entity to prevent clipping into walls. CARPET-XCOM
            if(CarpetSettings.reloadSuffocationFix){
                NBTTagList nbttaglist4 = compound.getTagList("AABBcarpet", 6);
                if(nbttaglist4 != null && nbttaglist4.tagCount() == 6) {
                    AxisAlignedBB aabb = new AxisAlignedBB(nbttaglist4.getDoubleAt(0), nbttaglist4.getDoubleAt(1), nbttaglist4.getDoubleAt(2), nbttaglist4.getDoubleAt(3), nbttaglist4.getDoubleAt(4), nbttaglist4.getDoubleAt(5));
                    if (aabb != null && !(aabb.minX == 0 && aabb.minY == 0 && aabb.minZ == 0 && aabb.maxX == 0 && aabb.maxY == 0 && aabb.maxZ == 0)) {
    ...
    {
        if (CarpetSettings.rideableGhasts && this.height > 2.0F)
        {
    ...
    public Entity changeDimension(int dimensionIn)
    {
        if (!this.world.isRemote && !this.isDead)
        {
            ...
            else
            {
                ...
                if(carpet.carpetclient.CarpetClientChunkLogger.logger.enabled)
                    carpet.carpetclient.CarpetClientChunkLogger.setReason("Entity going through nether portal: " + getName());
