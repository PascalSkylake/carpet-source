...
public class EntityPlayerMP extends EntityPlayer implements IContainerListener
{
            ...
            if (entity instanceof EntityCreeper)
            {
                ...
                if (entitycreeper.getPowered() && entitycreeper.ableToCauseSkullDrop())
                {
                    ...
                    try
                    {
                        ...
                        playerskull.setTagCompound(JsonToNBT.getTagFromJson(String.format("{SkullOwner:\"%s\"}", getName().toLowerCase())));
                        this.entityDropItem(playerskull, 0.0F);
                    }
    ...
    public void addStat(StatBase stat, int amount)
    {
        ...
        if (stat != null)
        {
            ...
            // CM
            if (stat instanceof StatSubItem) addStat(((StatSubItem) stat).getBase(), amount);
            // CM END
        }
    }
    ...
    public void setGameType(GameType gameType)
    {
        ...
        {
            if(CarpetSettings.spectatorsDontLoadChunks) ((WorldServer)world).getPlayerChunkMap().removePlayer(this);
            this.spawnShoulderEntities();
            this.dismountRidingEntity();
        }
        else
        {
            if(CarpetSettings.spectatorsDontLoadChunks) ((WorldServer)world).getPlayerChunkMap().addPlayer(this);
    ...
    {
        return this.interactionManager.getGameType() == GameType.SURVIVAL;
    }
    ...
    public void storeCameraData(boolean hasNightvision) {
        ...
        storeY = posY;
        storeZ = posZ;
        storeYaw = rotationYaw;
        storePitch = rotationPitch;
        storedDim = dimension;
    ...
    public boolean moveToStoredCameraData(){
        if(CarpetSettings.cameraModeRestoreLocation){
            if(storedDim != dimension){
                ...
                WorldServer worldserver2 = (WorldServer)server.getWorld(storedDim);
                dimension = storedDim;
                ...
                if(LoggerRegistry.__invisDebug){ // Added debugger for the complex bug turning players invisible. CARPET-XCOM
                    LoggerRegistry.getLogger("invisDebug").log(()-> new ITextComponent[]{
                            Messenger.s(null, "s2: " + worldserver1.loadedEntityList.contains(this) + " " + worldserver2.loadedEntityList.contains(this))
                    });
                }
                ...
                if (isEntityAlive())
                {
                    setLocationAndAngles(storeX, storeY, storeZ, storeYaw, storePitch);
                    worldserver2.spawnEntity(this);
                    worldserver2.updateEntityWithOptionalForce(this, false);
                }
                if(LoggerRegistry.__invisDebug){ // Added debugger for the complex bug turning players invisible. CARPET-XCOM
                    LoggerRegistry.getLogger("invisDebug").log(()-> new ITextComponent[]{
                            Messenger.s(null, "s3: " + worldserver1.loadedEntityList.contains(this) + " " + worldserver2.loadedEntityList.contains(this) + " " + isEntityAlive() + " " + this.getHealth())
                    });
                }
                setWorld(worldserver2);
                this.server.getPlayerList().preparePlayer(this, worldserver1);
                setPositionAndUpdate(storeX, storeY, storeZ);
                interactionManager.setWorld(worldserver2);
                this.server.getPlayerList().updateTimeAndWeatherForPlayer(this, worldserver2);
                this.server.getPlayerList().syncPlayerInventory(this);
                if(LoggerRegistry.__invisDebug){ // Added debugger for the complex bug turning players invisible. CARPET-XCOM
                    LoggerRegistry.getLogger("invisDebug").log(()-> new ITextComponent[]{
                            Messenger.s(null, "s4: " + worldserver1.loadedEntityList.contains(this) + " " + worldserver2.loadedEntityList.contains(this))
                    });
                }
                return true;
            }else {
                if(storeX == 0 && storeY == 0 && storeZ == 0) storeY = 256.0f;
                double dist = Math.sqrt(new BlockPos(storeX, storeY, storeZ).distanceSq(posX, posY, posZ));
                connection.setPlayerLocation(storeX, storeY, storeZ, storeYaw, storePitch);
