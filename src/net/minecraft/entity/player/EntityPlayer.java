...
public abstract class EntityPlayer extends EntityLivingBase
{
    ...
    public EntityItem dropItem(ItemStack droppedItem, boolean dropAround, boolean traceItem)
    {
        ...
        else
        {
            ...
            if (traceItem)
            {
                ...
                {
                    this.addStat(StatHelper.getDroppedObjectStats(itemstack.getItem(), itemstack.getMetadata()), droppedItem.getCount());
                }
    ...
    protected void damageEntity(DamageSource damageSrc, float damageAmount)
    {
        if (!this.isEntityInvulnerable(damageSrc))
        {
            ...
            DamageReporter.modify_damage(this, damageSrc, previous_amount, damageAmount, //CM
                    String.format("armour %.1f and toughness %.1f", (float)this.getTotalArmorValue(), (float)this.getEntityAttribute(SharedMonsterAttributes.ARMOR_TOUGHNESS).getAttributeValue())); //CM
    ...
    public boolean canPlayerEdit(BlockPos pos, EnumFacing facing, ItemStack stack)
    {
        ...
        else if (CarpetSettings.disableBedrockPlacement && stack.getItem() == Item.getItemFromBlock(Blocks.BEDROCK) && !this.capabilities.isCreativeMode)
        {
            return false;
        }
        else
