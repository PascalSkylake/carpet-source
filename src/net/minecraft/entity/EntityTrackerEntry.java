...
public class EntityTrackerEntry
{
    ...
    public void removeFromTrackedPlayers(EntityPlayerMP playerMP)
    {
        if(carpet.logging.LoggerRegistry.__invisDebug){ // Added debugger for the complex bug turning players invisible. CARPET-XCOM
            ...
            carpet.logging.LoggerRegistry.getLogger("invisDebug").log(()-> new net.minecraft.util.text.ITextComponent[]{
                    carpet.utils.Messenger.s(null, "r1: " + playerMP + " " + ss)
            });
        }
        ...
        if (playerMP != this.trackedEntity)
        {
            if (this.isVisibleTo(playerMP))
            {
                if (!this.trackingPlayers.contains(playerMP) && (this.isPlayerWatchingThisChunk(playerMP) || this.trackedEntity.forceSpawn))
                {
                    ...
                    if (!"false".equals(CarpetSettings.leashFix) && this.trackedEntity instanceof EntityLiving)
                    {
    ...
    public boolean isVisibleTo(EntityPlayerMP playerMP)
    {
        ...
        int i = Math.min(r, this.maxRange);
    ...
    private boolean isPlayerWatchingThisChunk(EntityPlayerMP playerMP)
    {
        ...
        if(CarpetSettings.spectatorsDontLoadChunks && playerMP.isSpectator()) return true;
