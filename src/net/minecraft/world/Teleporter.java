...
public class Teleporter
{
    ...
    public boolean placeInExistingPortal(Entity entityIn, float rotationYaw)
    {
        ...
        else if (CarpetSettings.portalCaching && this.destinationHistoryCache.containsKey(l)) // potential best candidate for linkage.
        {
            ...
            if (this.world.getBlockState(teleporter$portalposition_cm).getBlock() == Blocks.PORTAL)
        ...
        if (d0 >= 0.0D)
        {
            if (flag)
            {
                this.destinationCoordinateCache.put(l, new Teleporter.PortalPosition(blockpos, this.world.getTotalWorldTime(), new Vec3d(entityIn.posX, entityIn.posY, entityIn.posZ)));
            }
            /* carpet mod */
            if (CarpetSettings.portalCaching && (flag || flag_cm))
            {
                //its timeless
                this.destinationHistoryCache.put(l, new Teleporter.PortalPosition(blockpos, 0L, new Vec3d(entityIn.posX, entityIn.posY, entityIn.posZ)));
            }
            /* carpet mod end */
    ...
    public void clearHistoryCache()
    {
        ...
        for (int i=-1; i<2; i++)
        {
            WorldServer s = minecraftserver.getWorld(i);
            s.getDefaultTeleporter().removeAllCachedEntries();
        }
    }
