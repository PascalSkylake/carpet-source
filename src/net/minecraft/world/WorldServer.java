...
public class WorldServer extends World implements IThreadListener
{
    ...
    public final Set<NextTickListEntry> pendingTickListEntriesHashSet = Sets.<NextTickListEntry>newHashSet(); // CM: made public for doubleTileTickSchedulingFix
    /** All work to do in future ticks. */
    public final TreeSet<NextTickListEntry> pendingTickListEntriesTreeSet = new TreeSet<NextTickListEntry>(); // CM: made public for doubleTileTickSchedulingFix
    ...
    public WorldServer(MinecraftServer server, ISaveHandler saveHandlerIn, WorldInfo info, int dimensionId, Profiler profilerIn)
    {
        ...
        this.dimensionName = DimensionUtils.getId(this.provider.getDimensionType()).toString();
    }
    ...
    public void tick()
    {
        ...
        if(CarpetSettings.limitITTupdates > 0 && server.isCallingFromMinecraftThread()) {
        ...
        { // CM extra indent
        // RSMM start
        boolean tickTime = provider.getDimensionType() == DimensionType.OVERWORLD; 
		...
		if(LoggerRegistry.__rng){
            LoggerRegistry.getLogger("rng").log(()-> new ITextComponent[]{
                    Messenger.s(null, String.format("RNG TickUp. t:%d seed:%d d:%s", server.getTickCounter(), getRandSeed(), provider.getDimensionType().name()))
            });
        }
        ...
        if(LoggerRegistry.__rng){
            LoggerRegistry.getLogger("rng").log(()-> new ITextComponent[]{
                    Messenger.s(null, String.format("RNG BlockEv. t:%d seed:%d d:%s", server.getTickCounter(), getRandSeed(), provider.getDimensionType().name()))
            });
        }
        if(CarpetSettings.setSeed != 0){
        	this.rand.setSeed(CarpetSettings.setSeed ^ 0x5DEECE66DL);
    ...
    public void updateAllPlayersSleepingFlag()
    {
        ...
        if (!this.playerEntities.isEmpty())
        {
            ...
            { // custom amount
                allPlayersSleeping = j > 0;
            }
            else
            { // vanilla - all
                allPlayersSleeping = j > 0 && j >= this.playerEntities.size() - i;
            }
        }
    }
    ...
    protected void updateBlocks()
    {
        ...
        else
        {
            ...
            boolean overworldIceOnly = !CarpetSettings.enableStableLCGNetherEnd || provider.getDimensionType().getId() == 0; // Rule to disable nether and end ice snow and lightning Carpet-XCOM
            ...
            for (Iterator<Chunk> iterator = this.playerChunkMap.getChunkIterator(); iterator.hasNext(); this.profiler.endSection())
            {
                ...
                if (overworldIceOnly && flag && flag1 && this.rand.nextInt(100000) == 0)
                {
                ...
                if (overworldIceOnly && this.rand.nextInt(16) == 0)
                {
                    this.updateLCG = this.updateLCG * 3 + 1013904223;
                        ...
                        if (extendedblockstorage != Chunk.NULL_BLOCK_STORAGE && extendedblockstorage.needsRandomTick())
                        {
                            for (int i1 = 0; i1 < i; ++i1)
                            {
                                ...
                                if (block.getTickRandomly())
                                {
                                    ...
                                    int x = k1 + j;
                                    int z = l1 + k;
                                    int y = i2 + extendedblockstorage.getYLocation();
                                    BlockPos pos = new BlockPos(x, y, z);
                                    WorldHelper.onRandomTick(this, pos);
                                    // RSMM end

                                    block.randomTick(this, pos, iblockstate, this.rand);
                                }
    ...
    public boolean isBlockTickPending(BlockPos pos, Block blockType)
    {
        ...
        if(!CarpetSettings.doubleTileTickSchedulingFix){
            nextticklistentry = new NextTickListEntry(pos, blockType);
        }else{
            nextticklistentry = new NextTickListEntryFix(pos, blockType);
        }
    ...
    public boolean isUpdateScheduled(BlockPos pos, Block blk)
    {
        ...
        // Fix double tile tick scheduling with a java bug similar to how its fixed in MC1.13 and above CARPET-XCOM
        if(!CarpetSettings.doubleTileTickSchedulingFix){
            nextticklistentry = new NextTickListEntry(pos, blk);
        }else{
            nextticklistentry = new NextTickListEntryFix(pos, blk);
        }
    ...
    public void updateBlockTick(BlockPos pos, Block blockIn, int delay, int priority)
    {
        ...
        // Fix double tile tick scheduling with a java bug similar to how its fixed in MC1.13 and above CARPET-XCOM
        if(!CarpetSettings.doubleTileTickSchedulingFix){
            nextticklistentry = new NextTickListEntry(pos, blockIn);
        }else{
            nextticklistentry = new NextTickListEntryFix(pos, blockIn);
        }
    ...
    public void updateEntities()
    {
        ...
        if (!CarpetSettings.dimensionLoading && this.playerEntities.isEmpty())
        {
                ...
                while (iterator.hasNext())
                {
                    ...
                    if (this.isAreaLoaded(nextticklistentry1.position.add(0, 0, 0), nextticklistentry1.position.add(0, 0, 0)))
                    {
                        if (carpet.carpetclient.CarpetClientChunkLogger.logger.enabled)
                            carpet.carpetclient.CarpetClientChunkLogger.setReason("Block update: " + nextticklistentry1.getBlock().getLocalizedName());
    ...
    public void saveAllChunks(boolean all, @Nullable IProgressUpdate progressCallback) throws MinecraftException
    {
        ...
        if (chunkproviderserver.canSave())
        {
            ...
            for (Chunk chunk : Lists.newArrayList(chunkproviderserver.getLoadedChunks()))
            {
                ...
                {
                    if (!this.playerChunkMap.contains(chunk.x, chunk.z))
                ...
                else
                {
                    ...
                    if (!(entityIn instanceof EntityPlayer))
                    {
                        ...
                        if (CarpetSettings.sendDuplicateEntitiesToClients)
                            entityTracker.track(entityIn); // vanilla handles the un-tracking properly
                        return false;
                    }
    ...
    private boolean fireBlockEvent(BlockEventData event)
    {
        ...
        if (iblockstate.getBlock() == event.getBlock()) {
            WorldHelper.onBlockEvent(this, event);
        }
        // RSMM end
    ...
    public ArrayList<BlockEventData> getBlockEventQueue() {
        return blockEventQueue[this.blockEventCacheIndex];
    }
    public void setBlockEventQueue(ArrayList<BlockEventData> list) {
        for(BlockEventData data : list){
            blockEventQueue[this.blockEventCacheIndex].add(data);
        }
    }
}