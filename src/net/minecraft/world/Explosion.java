...
public class Explosion
{
    ...
    public final List<BlockPos> affectedBlockPositions = Lists.<BlockPos>newArrayList();
    /** Maps players to the knockback vector applied by the explosion, to send to the client */
    public final Map<EntityPlayer, Vec3d> playerKnockbackMap = Maps.<EntityPlayer, Vec3d>newHashMap();
    ...
    public Explosion(World worldIn, Entity entityIn, double x, double y, double z, float size, boolean causesFire, boolean damagesTerrain)
    {
        ...
        if (LoggerRegistry.__explosions) {
            this.logHelper = new ExplosionLogHelper(entityIn, x, y, z, size, causesFire);
        }
    }
    ...
    public void doExplosionA()
    {
        ...
        if(!CarpetSettings.explosionNoBlockDamage) this.affectedBlockPositions.addAll(set);
    ...
    public void doExplosionB(boolean spawnParticles)
    {
         ...
         */
        if (CarpetSettings.optimizedTNT) {
            OptimizedTNT.doExplosionB(this, spawnParticles);
            return;
        }
        ...
        if(LoggerRegistry.__explosions) {
            this.logHelper.onExplosionDone(this.world.getWorldTime());
        }
    }
