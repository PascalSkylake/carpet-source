...
public abstract class World implements IBlockAccess
{
    ...
    public boolean setBlockState(BlockPos pos, IBlockState newState, int flags, String reason)
    {
        ...
        boolean successful = setBlockState(pos, newState, flags);
    ...
    public boolean setBlockState(BlockPos pos, IBlockState newState, int flags)
    {
        ...
        else
        {
            ...
            IBlockState iblockstate = chunk.setBlockState_carpet(pos, newState, ((flags & 128) != 0) ?true:false ); // carpet added flag
            ...
            else
            {
                ...
                else if (!this.isRemote && ((flags & 16) == 0) && ((flags & 128) == 0))
                {
    ...
    public void notifyNeighborsOfStateChange(BlockPos pos, Block blockType, boolean updateObservers)
    {
        ...
        if (CarpetSettings.extendedConnectivity)
        {
            ...
            if (updateObservers)
            {
                this.updateObservingBlocksAt_other(posd, blockType);
            }
        }
        /* end */
    ...
    public void notifyNeighborsOfStateExcept(BlockPos pos, Block blockType, EnumFacing skipSide)
    {
        ...
        if (CarpetSettings.extendedConnectivity)
        {
            ...
            {
                this.neighborChanged(posd.west(), blockType, posd);
            }

            if (skipSide != EnumFacing.EAST)
            {
                this.neighborChanged(posd.east(), blockType, posd);
            }

            if (skipSide != EnumFacing.DOWN)
            {
                this.neighborChanged(posd.down(), blockType, posd);
            }
            ...
            {
                this.neighborChanged(posd.north(), blockType, posd);
            }

            if (skipSide != EnumFacing.SOUTH)
            {
                this.neighborChanged(posd.south(), blockType, posd);
            }
    ...
    public void neighborChanged(BlockPos pos, final Block blockIn, BlockPos fromPos)
    {
        if(CarpetSettings.updateSuppressionCrashFix){
            neighborChangedUpdateSuppressionFixed(pos, blockIn, fromPos);
            return;
        }
    ...
    {
        if(CarpetSettings.updateSuppressionCrashFix){
            observedNeighborChangedUpdateSuppressionFixed(pos, changedBlock, changedBlockPos);
            return;
        }
    ...
    {
        if(LoggerRegistry.__invisDebug && entityIn instanceof EntityPlayerMP){ // Added debugger for the complex bug turning players invisible. CARPET-XCOM
            ...
            LoggerRegistry.getLogger("invisDebug").log(()-> new ITextComponent[]{
                    Messenger.s(null, "r1: " + entityIn + " " + ss)
            });
        }
    ...
    private boolean getCollisionBoxes(@Nullable Entity entityIn, AxisAlignedBB aabb, boolean p_191504_3_, @Nullable List<AxisAlignedBB> outList)
    {
        if (CarpetSettings.blockCollisionsOptimization) { // CARPET-MASA
            return CollisionBoxesOptimizations.optimizedGetCollisionBoxes(this, entityIn, aabb, p_191504_3_, outList);
        }
    ...
    public void updateEntities()
    {
            ...
            if (entity2.isDead)
            {
                ...
                if (entity2.addedToChunk && this.isChunkLoaded(l1, i2, true))
                {
                    if(carpet.carpetclient.CarpetClientChunkLogger.logger.enabled)
                        carpet.carpetclient.CarpetClientChunkLogger.setReason("Removing entity from chunk: " + entity2.getName());
        ...
        if (CarpetSettings.optimizedTileEntityRemoval) {
            this.tickableTileEntities.removeAll(tileEntitiesRemovedInTick);
            this.loadedTileEntityList.removeAll(tileEntitiesRemovedInTick);
        }
    ...
    public void updateEntityWithOptionalForce(Entity entityIn, boolean forceUpdate)
    {
        ...
        if (forceUpdate && entityIn.addedToChunk)
        {
            ...
            if (entityIn.isRiding())
            {
                if(!CarpetSettings.commandLazyChunkBehavior || LazyChunkBehaviorHelper.shouldUpdate(entityIn)) {
            ...
            {
                if(!CarpetSettings.commandLazyChunkBehavior || LazyChunkBehaviorHelper.shouldUpdate(entityIn)) {
                    WorldHelper.onEntityTick(this, entityIn); // RSMM
                    entityIn.onUpdate();
                }
            }
        }
        ...
        if (!entityIn.addedToChunk || entityIn.chunkCoordX != i3 || entityIn.chunkCoordY != j3 || entityIn.chunkCoordZ != k3)
        {
            ...
            if (!CarpetSettings.unloadedEntityFix && !entityIn.setPositionNonDirty() && !this.isChunkLoaded(i3, k3, true))
            {
    ...
    public boolean checkNoEntityCollision(AxisAlignedBB bb, @Nullable Entity entityIn)
    {
        ...
        for (int j2 = 0; j2 < list.size(); ++j2)
        {
            ...
            if (!entity4.isDead && entity4.preventEntitySpawning && entity4 != entityIn && (entityIn == null || entity4.isRidingSameEntity(entityIn) || CarpetSettings._1_8Spawning))
            {
                return false;
            }
        }
    ...
    public boolean mayPlace(Block blockIn, BlockPos pos, boolean skipCollisionCheck, EnumFacing sidePlacedOn, @Nullable Entity placer)
    {
        ...
        if (!CarpetSettings.ignoreEntityWhenPlacing && axisalignedbb != Block.NULL_AABB && !this.checkNoEntityCollision(axisalignedbb.offset(pos), placer))
        {
            return false;
        }
    ...
    public void updateComparatorOutputLevel(BlockPos pos, Block blockIn)
    {
        ...
        if (CarpetSettings.optimizedTileEntities)
        {
            ...
            TileEntityOptimizer.updateComparatorsAndLazyTileEntities(this, pos, blockIn);
        }
        else
        {
    ...
    public long getRandSeed(){
        return SEED_ACCESSOR.get(this.rand).get();
    }
    ...
    public boolean isRemovableBlock(BlockPos pos) {
        if (this.isOutsideBuildHeight(pos)) {
            ...
            return false;
        } else if (getBlockState(pos) == Blocks.AIR.getDefaultState()) {
            return false;
        }
