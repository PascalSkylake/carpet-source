...
public class Chunk
{
    ...
    public Chunk(World worldIn, int x, int z)
    {
        ...
        this.tileEntities = CarpetSettings.reloadUpdateOrderFix ? Maps.newLinkedHashMap() : Maps.newHashMap(); // CM: reload update order fix
    ...
    private void relightBlock(int x, int y, int z)
    {
        ...
        {
        	// NewLight PHIPRO-CARPET
        	if (!CarpetSettings.newLight) this.world.markBlocksDirtyVertical(x + this.x * 16, z + this.z * 16, j, i); //Forge: Useless, since heightMap is not updated yet (See #3871)
            ...
            if (this.world.provider.hasSkyLight())
            {
            	// NewLight PHIPRO-CARPET
            	if (CarpetSettings.newLight){
            		LightingHooks.relightSkylightColumn(this.world, this, x, z, i, j); //Forge: Optimized version of World.markBlocksDirtyVertical; heightMap is now updated (See #3871)
    ...
    {
        return setBlockState_carpet(pos, state, false);
    }
    ...
    public IBlockState setBlockState_carpet(BlockPos pos, IBlockState state, boolean skip_updates) //CM added skip_updates
    {
        ...
        else
        {
            ...
            if (extendedblockstorage == NULL_BLOCK_STORAGE)
            {
                ...
                // NewLight PHIPRO-CARPET
            	if (CarpetSettings.newLight){
            		LightingHooks.initSkylightForSection(this.world, this, extendedblockstorage); //Forge: Always initialize sections properly (See #3870 and #3879)
            	}
            }
            ...
            if (CarpetSettings.redstoneMultimeter && !world.isRemote) {
                WorldHelper.getMultimeter().onBlockChange(world, pos, iblockstate, state);
            }
            // RSMM end

            if (block1 != block)
            {
                if (!skip_updates && !this.world.isRemote)
                {
            ...
            else
            {
                ...
                if (!skip_updates && !this.world.isRemote && block1 != block)
                {
                ...
                this.dirty = true;
                // ----- RSMM Start ----- //
                if (CarpetSettings.redstoneMultimeterLegacy) { StateChangeEventDispatcher.dispatchEvent(this.getWorld(), pos); }
                // ----- RSMM End ----- //
                return iblockstate;
            }
        }
    ...
    {
    	if (CarpetSettings.newLight) this.world.lightingEngine.procLightUpdates(type);
        int i = pos.getX() & 15;
        int j = pos.getY();
        int k = pos.getZ() & 15;
        ExtendedBlockStorage extendedblockstorage = this.storageArrays[j >> 4];
    ...
    public void setLightFor(EnumSkyBlock type, BlockPos pos, int value)
    {
        ...
        if (extendedblockstorage == NULL_BLOCK_STORAGE)
        {
            ...
            // NewLight PHIPRO-CARPET
    		if (CarpetSettings.newLight){
            	LightingHooks.initSkylightForSection(this.world, this, extendedblockstorage); //Forge: generateSkylightMap produces the wrong result (See #3870)
            }else{
            	this.generateSkylightMap();
            }
        }
    ...
    public void onLoad()
    {
        ...
        // NewLight PHIPRO-CARPET
     	if (CarpetSettings.newLight){
     		LightingHooks.onLoad(this.world, this);
     	}
    }
    ...
    protected void populate(IChunkGenerator generator)
    {
        if (this.isTerrainPopulated())
        {
            if (generator.generateStructures(this, this.x, this.z))
            {
                ...
                if(CarpetClientChunkLogger.logger.enabled) {
                	...
                	CarpetClientChunkLogger.logger.log(this.world,this.x,this.z,CarpetClientChunkLogger.Event.GENERATING_STRUCTURES);
        ...
        else
        {
            ...
            if(CarpetClientChunkLogger.logger.enabled) {
            	...
            	CarpetClientChunkLogger.logger.log(this.world,this.x,this.z,CarpetClientChunkLogger.Event.POPULATING);
                carpet.carpetclient.CarpetClientChunkLogger.resetReason();
            }
            ...
            if(CarpetSettings.skyblock) {
                for(int i = 0; i < 4; i++) {
                    if(world.isChunkGeneratedAt(x + i % 2, z + i / 2)) {
                        world.getChunk(x + i % 2, z + i / 2).removeAllBlocks();
                    }
                }
            }
            this.markDirty();
    ...
    public void removeAllBlocks(){
                ...
                for (int l = 0; l < 256; ++l)
                {
                    ...
                    try {
                        if(this.storageArrays[i1] != null && this.storageArrays[i1].get(j, l & 15, k).getBlock() != Blocks.END_PORTAL_FRAME){
                            ...
                            if(this.storageArrays[i1].getSkyLight() != null) this.storageArrays[i1].setSkyLight(j, l & 15, k, 15);
                        }
                    }catch(Exception e){
                    }
                }
        ...
        if(entityLists != null){
            ...
            for(int iter = 0; iter < entityLists.length; iter++){
                Iterator<Entity> iterator = entityLists[iter].iterator();
