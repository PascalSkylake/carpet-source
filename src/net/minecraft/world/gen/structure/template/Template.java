...
public class Template
{
                    ...
                    if ((block == null || block != block1) && (!placementIn.getIgnoreStructureBlock() || block1 != Blocks.STRUCTURE_BLOCK) && (structureboundingbox == null || structureboundingbox.isVecInside(blockpos)))
                    {
                        ...
                        if (template$blockinfo1.tileentityData != null)
                        {
                            ...
                            if (tileentity != null)
                            {
                                ...
                                worldIn.setBlockState(blockpos, Blocks.BARRIER.getDefaultState(), 4 | (CarpetSettings.fillUpdates?0:128)); //CM
                            }
                        }

                        if (worldIn.setBlockState(blockpos, iblockstate1, flags | (CarpetSettings.fillUpdates?0:128)) && template$blockinfo1.tileentityData != null) //CM
                        {
    ...
    private void addEntitiesToWorld(World worldIn, BlockPos pos, Mirror mirrorIn, Rotation rotationIn, @Nullable StructureBoundingBox aabb)
    {
        if(CarpetSettings.structuresReplaceEntities){
            ...
            for(Entity e : worldIn.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(pos, new BlockPos(transformedVec3d((new Vec3d(pos.add(size))) , mirrorIn, rotationIn))), NOT_PLAYER) ){
