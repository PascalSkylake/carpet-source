    ...
    private static void initCraftableStats()
    {
        ...
        for (Item item : set)
        {
            if (item != null)
            {
                ...
                {
                    // CM
                    StatCrafting baseStat = (StatCrafting) new StatCrafting("stat.craftItem.", s, new TextComponentTranslation("stat.craftItem", new ItemStack(item).getTextComponent()), item).registerStat();
                    CRAFTS_STATS[i] = baseStat;
    ...
    private static void initMiningStats()
    {
        for (Block block : Block.REGISTRY)
        {
            ...
            if (item != Items.AIR)
            {
                ...
                {
                    // CM
                    StatCrafting baseStat = (StatCrafting) new StatCrafting("stat.mineBlock.", s, new TextComponentTranslation("stat.mineBlock", new ItemStack(block).getTextComponent()), item).registerStat();
                    BLOCKS_STATS[i] = baseStat;
    ...
    private static void initStats()
    {
        for (Item item : Item.REGISTRY)
        {
            if (item != null)
            {
                ...
                {
                    // CM
                    StatCrafting baseStat = (StatCrafting) new StatCrafting("stat.useItem.", s, new TextComponentTranslation("stat.useItem", new ItemStack(item).getTextComponent()), item).registerStat();
                    OBJECT_USE_STATS[i] = baseStat;
                    if (!(item instanceof ItemBlock)) USE_ITEM_STATS.add((StatCrafting)OBJECT_USE_STATS[i]);
    ...
    private static void initPickedUpAndDroppedStats()
    {
        for (Item item : Item.REGISTRY)
        {
            if (item != null)
            {
                ...
                {
                    // CM
                    StatCrafting basePickupStat = (StatCrafting) new StatCrafting("stat.pickup.", s, new TextComponentTranslation("stat.pickup", new ItemStack(item).getTextComponent()), item).registerStat();
                    OBJECTS_PICKED_UP_STATS[i] = basePickupStat;
                    StatCrafting baseDropStat = (StatCrafting) new StatCrafting("stat.drop.", s, new TextComponentTranslation("stat.drop", new ItemStack(item).getTextComponent()), item).registerStat();
                    OBJECTS_DROPPED_STATS[i] = baseDropStat;
