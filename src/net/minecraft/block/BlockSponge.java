...
public class BlockSponge extends Block
{
    ...
    public void randomTick(World worldIn, BlockPos pos, IBlockState state, Random random)
    {
        ...
        for (EnumFacing enumfacing : EnumFacing.values())
        {
            ...
            if (worldIn.getBlockState(blockpos1).getBlock() == Blocks.SPONGE && worldIn.getBlockState(blockpos1).getValue(WET).booleanValue() )
            {
                touches_wet = true;
            }
        }
        if ((state.getValue(WET)).booleanValue() && !touches_water && worldIn.canSeeSky(pos.up()) && worldIn.isDaytime() && !worldIn.isRainingAt(pos.up()))
        {
            worldIn.setBlockState(pos, state.withProperty(WET, Boolean.valueOf(false)), 2);
        }
        else if ( !(state.getValue(WET)).booleanValue() && (touches_wet || touches_water || worldIn.isRainingAt(pos.up())) && random.nextInt(3)==0)
        {
            worldIn.setBlockState(pos, state.withProperty(WET, Boolean.valueOf(true)), 2);
        }
    }
