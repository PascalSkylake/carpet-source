...
public class Block implements IBlock /* RSMM */
{
    ...
    public void randomTick(World worldIn, BlockPos pos, IBlockState state, Random random)
    {
        if(carpet.carpetclient.CarpetClientChunkLogger.logger.enabled)
            carpet.carpetclient.CarpetClientChunkLogger.setReason("Randomtick block: " + state.getBlock().getLocalizedName());
    ...
    public static void spawnAsEntity(World worldIn, BlockPos pos, ItemStack stack)
    {
        if (!worldIn.isRemote && !stack.isEmpty() && worldIn.getGameRules().getBoolean("doTileDrops"))
        {
            ...
            if(CarpetSettings.carefulBreak && PlayerInteractionManager.playerMinedBlock != null && PlayerInteractionManager.playerMinedBlock.isSneaking() && LoggerRegistry.getLogger("carefulBreak").subscribed(PlayerInteractionManager.playerMinedBlock)){
                entityitem.onCollideWithPlayer(PlayerInteractionManager.playerMinedBlock);
    ...
    public void harvestBlock(World worldIn, EntityPlayer player, BlockPos pos, IBlockState state, @Nullable TileEntity te, ItemStack stack)
    {
        ...
        player.addStat(StatHelper.getBlockStateStats(state));
        // CM END
    ...
    public static void registerBlocks()
    {
        ...
        registerBlock(35, "wool", (new BlockWool()).setHardness(0.8F).setSoundType(SoundType.CLOTH).setTranslationKey("cloth"));
