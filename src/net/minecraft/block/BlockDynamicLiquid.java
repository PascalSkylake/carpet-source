...
public class BlockDynamicLiquid extends BlockLiquid
{
    ...
    public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand)
    {
        ...
        {
            Set<EnumFacing> set = this.getPossibleFlowDirections(worldIn, pos, i);
            int k1 = i + j;
    ...
    private Set<EnumFacing> getPossibleFlowDirections(World worldIn, BlockPos pos, int level)
    {
        ...
        int maxflow = 1;
        if (CarpetSettings.waterFlow != CarpetSettings.WaterFlow.vanilla && level < 8)
        {
            maxflow = level - (CarpetSettings.waterFlow == CarpetSettings.WaterFlow.correct ? 1 : 3); // level = 7 is the shallowest water
        ...
        for (EnumFacing enumfacing : EnumFacing.Plane.HORIZONTAL)
        {
            ...
            if (!this.isBlocked(worldIn, blockpos, iblockstate) && (iblockstate.getMaterial() != this.material || ((Integer)iblockstate.getValue(LEVEL)).intValue() > 0))
            {
                ...
                {
                    j = this.getSlopeDistance(worldIn, blockpos, maxflow, enumfacing.getOpposite()); // maxflow = 1
                }
                else
                {
                    j = 0;
