    ...
    public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand)
    {
        if (!this.isLocked(worldIn, pos, state))
        {
            ...
            else if (!this.isRepeaterPowered)
            {
                ...
                if (!flag)
                {
                    worldIn.updateBlockTick(pos, this.getPoweredState(state).getBlock(), this.getTickDelay(state, worldIn, pos), -1);
                }
            }
        }
    }
    ...
    protected void updateState(World worldIn, BlockPos pos, IBlockState state)
    {
        if (!this.isLocked(worldIn, pos, state))
        {
            ...
            if (this.isRepeaterPowered != flag && !worldIn.isBlockTickPending(pos, this))
            {
                ...
                worldIn.updateBlockTick(pos, this, this.getDelay(state, worldIn, pos), i);
            }
        }
    }
