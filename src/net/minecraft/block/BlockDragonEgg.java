...
public class BlockDragonEgg extends Block
{
    ...
    private void checkFall(World worldIn, BlockPos pos)
    {
        if (BlockFalling.canFallThrough(worldIn.getBlockState(pos.down())) && pos.getY() >= 0)
        {
            ...
            if ((!BlockFalling.fallInstantly && worldIn.isAreaLoaded(pos.add(-32, -32, -32), pos.add(32, 32, 32)))
                    && (!CarpetSettings.commandLazyChunkBehavior || LazyChunkBehaviorHelper.shouldUpdate(worldIn, pos))) {
    ...
    private boolean isMeat(Item food)
    {
        ...
        if(food == Items.FISH){ return true; }
        if(food == Items.COOKED_FISH){ return true; }
